...
TODO:
-sort countries according to value for each category
-make year selector a visual timeline
 */
//=============START ==============//
let toolTip,
		x,
...
function selectDataset() {
		...
		unit = categories
				.find(cat => currentCategory == cat.name)
				.unit;
		$('#unit').text(unit);
...
function changeYear(year) {
		currentYear = year
		getYearData(currentYear).then((graphdata) => {
				updateGraph(graphdata)
		})
}
...
function getCategoryData(category) {
		...
		// clone object and remove uneeded data (the deleted countries aren't officially
		// in the OECD)
...
function updateGraph(dataset) {
		...
		let upperBound = d3.max(dataset, function (d) {
				return d.value;
		});
		let lowerBound = d3.min(dataset, function (d) {
				return d.value * .9
		});
